    ...
    >
      ...
      >
        <svg ref="topRulerSvg" width="100%" height="20" class="block">
          <g :transform="`translate(${pageOffsetX - LEFT_RULER_WIDTH}, 0)`">
          ...
          <div class="flex-grow min-w-0">
            ...
            >
              ...
              >
                ...
                <rect width="100%" height="100%" fill="url(#composite-grid)" />
              </svg>
              ...
              <div
                ...
                :class="{
                  'ring-1 ring-indigo-400': band.id === store.selectedBandId,
                }"
              ...
              >
                ...
                <div
                  ...
                  :class="{
                    'ring-2 ring-indigo-500':
                      store.selectedCompositeNodeId === root.id,
                    'cursor-grab': true,
                  }"
                ...
                >
                  ...
                  <!-- Resize handles -->
                  <template v-if="store.selectedCompositeNodeId === root.id">
                    <div
                      v-for="handle in RESIZE_HANDLES"
                      :key="handle.corner"
                      class="absolute z-10 h-2 w-2 border border-indigo-500 bg-white"
                      :style="handleStyle(handle.corner)"
                      @mousedown.stop="onRootResizeStart($event, root, handle)"
                    />
                  </template>
                </div>
              </div>
...
function duplicateCompositeRootById(rootId: string) {
  ...
  for (const band of bands) {
    if (!band.children) continue;
    const idx = band.children.findIndex((r) => "node" in r && r.id === rootId);
    if (idx === -1) continue;
...
function bandStyle(band: ReportBand): Record<string, string> {
  ...
  const widthPx =
    (store.page.width - store.page.margin.left - store.page.margin.right) * s;
...
const { onResizeStart: onChildResizeRaw } = useResize({
  ...
  apply: ({ dx, dy }) => {
    ...
    const delta =
      (childResizeDimension === "width" ? dx : dy) * childResizeDirection;
...
function onChildResizeStart(
  ...
  childResizeOrigValue = getNodeDimension(
