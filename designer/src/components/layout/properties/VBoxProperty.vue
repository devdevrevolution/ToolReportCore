...
import type { VBoxNode } from '@/types/designer'
...
const emit = defineEmits<{
    update: [id: string, patch: Partial<VBoxNode>]
}>()

const { parseDimension } = usePropertyHelpers()

function update(patch: Partial<VBoxNode>) {
...
<template>
    <PropertyGroup title="Container" collapsible>
        <div class="grid grid-cols-2 gap-2">
            <div>
                <label class="mb-1 block text-xs font-medium text-gray-700">Padding (mm)</label>
                <input
                    type="number"
                    ...
                    min="0"
                    @change="update({ padding: Number(($event.target as HTMLInputElement).value) })"
                />
            </div>
            <div>
