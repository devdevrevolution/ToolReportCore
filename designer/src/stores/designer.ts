...
function allElements(bands: ReportBand[]): DesignerElement[] {
  return bands.flatMap((b) =>
    flattenChildren(b.children.filter(isDesignerChild)),
  );
}
...
function removeFromContainerChildren(
...
function findElementInBands(
  ...
  for (const band of bands) {
    const idx = band.children.findIndex(
      (el) => el.id === id && isDesignerChild(el),
    );
    if (idx !== -1) {
      return {
...
function findInContainerChildren(
  ...
  for (const el of elements) {
    if (el.type === "container") {
      ...
      if (idx !== -1) {
        return {
          element: content.children[idx],
          parentContainerId: el.id,
          containerChildIndex: idx,
        };
      }
...
function hydrateElement(element: Partial<DesignerElement>): DesignerElement {
  ...
  if (hydrated.type === "container" && hydrated.content?.type === "container") {
    ...
    hydrated.content = {
      ...hydrated.content,
      children: (containerContent.children ?? []).map((child) =>
...
function collectConflicts(node: CompositeNode, conflicts: Set<string>): void {
  if ("children" in node && node.children) {
    ...
    const siblingsSum = node.children.reduce(
      (s, c) => s + getNodeSize(c, mainAxis),
      0,
    );
    ...
    for (const child of node.children) {
      ...
      const isAutoCross =
        child.type === "Label" &&
        childCross === (crossAxis === "width" ? 40 : 20);
 ...
 */
function findCompositeRoot(
  bands: ReportBand[],
  rootId: string,
): CompositeRoot | null {
  for (const band of bands) {
    if (!band.children) continue;
    const root = band.children.find(
      (r): r is CompositeRoot => isCompositeRoot(r) && r.id === rootId,
    );
    if (root) return root;
  }
  return null;
...
function deepCloneCompositeRoot(root: CompositeRoot): CompositeRoot {
  function cloneNode(node: CompositeNode): CompositeNode {
    ...
    if ("children" in cloned && Array.isArray(cloned.children)) {
      cloned.children = cloned.children.map((child: CompositeNode) =>
        cloneNode(child),
      );
    }
  ...
  if ("children" in node && node.children) {
    return (
      node.children.reduce(
        (sum, child) => sum + compositeNodeHeight(child),
        0,
      ) + 3
    );
  }
...
export const useDesignerStore = defineStore("store/designer", {
  ...
  getters: {
      ...
      return state.page.bands.some((band) =>
        (band.children ?? []).some(
          (el) =>
            isDesignerChild(el) &&
            el.type === "container" &&
            (el.content as ContainerContent).children?.some(
    ...
    bandRenderHeight:
      ...
      (bandId: string): number => {
        ...
        return Math.max(
          0,
          contentBottom - contentTop - topHeight - bottomHeight,
        );
      },
    ...
    designBandYPos:
      (state) =>
      (bandId: string): number => {
        const bands = (state.page.bands ?? []).filter(
          (b) => b.enabled !== false,
        );
    ...
    selectedBand: (state): ReportBand | null => {
      ...
      return (
        state.page.bands.find((b) => b.id === state.selectedBandId) ?? null
      );
    },
    ...
    selectedCompositeNode: (state): CompositeRoot | CompositeNode | null => {
      ...
      for (const band of state.page.bands) {
        if (!band.children) continue;
        const root = band.children
          .filter(isCompositeRoot)
          .find((r) => r.id === id);
        if (root) return root;
      }
    ...
    hasCompatibleClipboard: (state): boolean => {
      if (state.clipboard) {
        return Date.now() - state.clipboard.timestamp <= 24 * 60 * 60 * 1000;
      }
      try {
  ...
  actions: {
    ...
    restoreBands(snapshot: string): void {
      ...
      if (this.page.bands) {
        ...
        for (const restoredBand of restored) {
          const existing = this.page.bands.find(
            (b) => b.id === restoredBand.id,
          );
          if (existing) {
    ...
    elementBand(id: string): ReportBand | null {
      ...
      if (!bands) return null;
      return (
        bands.find((b) => (b.children ?? []).some((el) => el.id === id)) ?? null
      );
    },
    ...
    addElement(
      ...
      if (!band) {
        throw new Error(
          `[designer] Cannot add element — band "${targetBandId}" not found`,
        );
      }
    ...
    moveElements(ids: string[], dx: number, dy: number): void {
      ...
      for (const id of ids) {
        ...
        const contentWidth =
          this.page.width - this.page.margin.left - this.page.margin.right;

        const newX = Math.max(
          0,
          Math.min(contentWidth - found.element.width, found.element.x + dx),
        );
        const newY = Math.max(
          0,
          Math.min(bHeight - found.element.height, found.element.y + dy),
        );
    ...
    reorderElement(fromIndex: number, toIndex: number, bandId?: string): void {
      ...
      const targetBand = bandId ? bands.find((b) => b.id === bandId) : null;
      if (!targetBand) return;
    ...
    reorderBand(fromIndex: number, toIndex: number): void {
      ...
      if (
        fromIndex < 0 ||
        fromIndex >= bands.length ||
        toIndex < 0 ||
        toIndex >= bands.length
      ) {
        return;
      }
    ...
    duplicateElement(id: string): string {
      ...
      if (!source) {
        throw new Error(
          `[designer] Cannot duplicate — element ${id} not found`,
        );
      }
    ...
    addChildElement(
      ...
      if (!found)
        throw new Error(`[designer] Container ${containerId} not found`);
      if (found.element.type !== "container")
        throw new Error(`[designer] Element ${containerId} is not a container`);
    ...
    loadTemplate(template: PdfTemplate): void {
      ...
      const bandsHaveElements = loadedBands?.some(
        (b) => Array.isArray(b.children) && b.children.length > 0,
      );
      ...
      this.page = {
        ...
        orientation:
          (pageData.orientation as DesignerPage["orientation"]) ??
          defaultPage.orientation,
      ...
      this.discoveredFields =
        (configData?.discoveredFields as DiscoveredField[]) ?? [];
    ...
    saveTemplate(): CreateTemplatePayload {
      ...
      if (this.page.bands && this.page.bands.length > 0) {
        page.bands = this.page.bands.map((b) => {
          ...
          // Keep all BandChild types (CompositeRoot + DesignerElement)
    ...
    setBandHeight(bandId: string, height: number): void {
      ...
      const printableHeight =
        this.page.height - this.page.margin.top - this.page.margin.bottom;
    ...
    addCompositeNode(bandId: string, type: CompositeNodeType): string {
      ...
      const bands = this.page.bands;
      if (!bands)
        throw new Error("[designer] Cannot add composite node — no bands");
      ...
      if (node.type === "VBox" || node.type === "HBox") {
        ...
        if (!(node as { height?: number }).height) {
          ...
          (node as { height?: number }).height = Math.max(
            5,
    ...
    addChildCompositeNode(
      ...
      if (parent.type === "HBox" || parent.type === "VBox") {
        ...
        const siblingsSum = parent.children.reduce(
          (sum, c) => sum + getNodeSize(c, mainAxis),
          0,
        );
    ...
    updateCompositeNode(nodeId: string, patch: Partial<CompositeNode>): void {
      ...
      for (const band of bands) {
        if (!band.children) continue;
        const rootIdx = band.children.findIndex(
          (c) => c.id === nodeId && isCompositeRoot(c),
        );
        if (rootIdx !== -1) {
      ...
      for (const band of bands) {
        ...
        for (const root of band.children.filter(isCompositeRoot)) {
          ...
          if (root.node.id === nodeId) {
            ...
            if ("width" in patch)
              root.width = patch.width as number | undefined;
            if ("height" in patch)
              root.height = patch.height as number | undefined;
            this.isDirty = true;
            return;
          }
    ...
    resizeCompositeRoot(
      ...
      if (
        root.node.type === "VBox" ||
        root.node.type === "HBox" ||
        root.node.type === "Label" ||
        root.node.type === "Image" ||
        root.node.type === "Shape"
      ) {
          ...
          for (const child of children) {
            if (factor < 1) {
              const newMain = Math.max(
                5,
                Math.round(getNodeSize(child, mainAxis) * factor),
              );
    ...
    removeCompositeNode(nodeId: string): void {
      ...
      for (const band of bands) {
        ...
        const fullIdx = band.children.findIndex(
          (c) => c.id === nodeId && isCompositeRoot(c),
        );
        if (fullIdx !== -1) {
      ...
      for (const band of bands) {
        ...
        if ("children" in band.content) {
          const idx = band.content.children.findIndex(
            (c: CompositeNode) => c.id === nodeId,
          );
          if (idx !== -1) {
    ...
    pasteCompositeRoot(targetBandId?: string): string | null {
      ...
      // 2) Discard stale clipboard (>24h)
      if (Date.now() - entry.timestamp > 24 * 60 * 60 * 1000) {
        this.clipboard = null;
        try {
          localStorage.removeItem("pdf-designer-clipboard");
        } catch {}
        return null;
      }
      ...
      const bands = this.page.bands;
      if (!bands) return null;
      let band: ReportBand | null =
        bands.find((b) => b.id === targetBandId) ?? null;
